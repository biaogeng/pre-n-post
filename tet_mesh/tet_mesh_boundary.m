function boundary_element_node=tet_mesh_boundary (coord, conn)
...
%
%  Modified:
%    Biao Geng, 2018-10-15
%    changed input from file name to mesh arrays
%       node_xyz: coordinates
%       conn: element connectivity
%       
%    removed file output
%    add function return of surface element connectivity (in original nodal
%    index)
%
  fprintf ( 1, '\n' );
  timestamp ( );
...
%
  ...
  element_order = size(conn,2);
  element_num = size(conn,1);
  element_node = conn';    
...
%
  ...
  boundary_element_node=boundary_element_node';
%
%  Terminate.
%
  fprintf ( 1, '\n' );
